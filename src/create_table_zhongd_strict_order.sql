-- 严格按照提供字段顺序创建MySQL建表语句
-- 表名：中高端客户量质构效指标数据表（严格字段顺序）

CREATE TABLE zhongd_customer_metrics_strict (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    city_name VARCHAR(50) NOT NULL COMMENT '地市',
    
    -- 严格按照原始字段顺序定义
    zhongd_revenue BIGINT COMMENT '中高端客户收入-万元',
    photo_zhongd_revenue BIGINT COMMENT '拍照中高端客户收入-万元',
    ...
    zhongd_value_retention_rate BIGINT COMMENT '中高端客户价值保拓率-%',
    cmcc_app_active_rate BIGINT COMMENT '中国移动APP当月活跃率-%',
    domestic_roaming_rate BIGINT COMMENT '国漫渗透率-%',
    inter_province_roaming_rate BIGINT COMMENT '常漫客户（省际漫游）渗透率-%',
    call_10086_in_rate BIGINT COMMENT '10086呼入客户占比-%',
    call_10086_out_rate BIGINT COMMENT '10086呼出客户占比-%',
    complaint_rate BIGINT COMMENT '投诉客户占比-%',
    service_cancel_rate BIGINT COMMENT '业务退订客户占比-%',
    activity_continue_rate BIGINT COMMENT '活动接续客户占比-%',
    debt_rate BIGINT COMMENT '欠费客户占比-%',
    zhongd_fixed_mobile_fusion_rate BIGINT COMMENT '中高端客户固移融合率',
    zhongd_effective_fusion_rate BIGINT COMMENT '中高端客户有效融合率',
    abnormal_customer_rate BIGINT COMMENT '异动客户占比',
    elderly_customer_rate BIGINT COMMENT '银发客户占比',
    international_customer_rate BIGINT COMMENT '国际客户占比',
    campus_customer_rate BIGINT COMMENT '校园客户占比',
    ...
    total_decrease_value BIGINT COMMENT '下降总价值',
    value_increase_customers BIGINT COMMENT '价值提升客户',
    total_increase_value BIGINT COMMENT '提升总价值',
    ...
    photo_zhongd_tarpu BIGINT COMMENT '拍照中高端客户tarpu',
    photo_zhongd_dou BIGINT COMMENT '拍照中高端客户dou',
    photo_zhongd_monthly_traffic_saturation BIGINT COMMENT '拍照中高端客户通用包月流量饱和度',
    photo_zhongd_outgoing_mou BIGINT COMMENT '拍照中高端客户主叫mou',
    target_zhongd_tarpu BIGINT COMMENT '到达中高端客户tarpu_元',
    target_zhongd_dou BIGINT COMMENT '到达中高端客户dou_gb',
    target_zhongd_monthly_traffic_saturation BIGINT COMMENT '到达中高端客户通用包月流量饱和度',
    target_zhongd_outgoing_mou BIGINT COMMENT '到达中高端客户主叫mou_分钟',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='中高端客户量质构效指标数据表-严格字段顺序';

-- 字段对应关系（严格按照提供顺序）
/*
原始字段顺序 -> 数据库字段名：
1. 中高端客户收入-万元 -> zhongd_revenue
2. 拍照中高端客户收入-万元 -> photo_zhongd_revenue
3. 当前中高端客户-户 -> current_zhongd_customers
4. 中高端净增客户-户 -> net_increase_zhongd_customers
5. 当月新增中高端客户-户 -> monthly_new_zhongd_customers
6. 当月新增中高端客户（存量流入）-户 -> monthly_new_zhongd_stock_inflow
7. 当月新增中高端客户（新入网）-户 -> monthly_new_zhongd_new_access
8. 当月流出中高端客户-户 -> monthly_outflow_zhongd_customers
9. 当月离网中高端客户-户 -> monthly_offline_zhongd_customers
10. 当月离网中高端客户（携出）-户 -> monthly_offline_zhongd_portout
... (以此类推，共113个业务字段)
*/